...
const initialChatMessages: ChatMessage[] = [ { sender: 'bot', type: 'options', content: { text: '¡Hola! Soy OvidioBot, tu asistente virtual de Mundoicopor. ¿Cómo puedo ayudarte hoy?', options: [ { label: 'Ver Cielorrasos', query: 'show-ceilings' }, { label: 'Cotizar un proyecto', query: 'quote' }, { label: 'Hablar con un asesor', query: 'contact' }, ] } }
];
                ...
                <div className="glassmorphic p-4 sm:p-6 text-gray-800 max-h-[90vh] overflow-y-auto">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                        ...
                        <div className="flex flex-col">
                            ...
                            <button onClick={handleAddToCart} className="neumorphic-button w-full bg-blue-600 text-white font-bold py-3 px-6 rounded-full hover:bg-blue-700 transition-colors mt-4 text-sm sm:text-base">Añadir a Cotización</button>
                        </div>
                    </div>
...
const App: React.FC = () => {
    ...
    const [hoveredIcoporIndex, setHoveredIcoporIndex] = useState<number | null>(null);
    ...
    const handleSendMessage = (message: string, query?: string) => {
        ...
        setTimeout(() => {
            ...
            if (userQuery === 'show-ceilings' || userQuery.includes('cielorraso')) {
                ...
                const optionsResponse: ChatMessage = { sender: 'bot', type: 'options', content: { text: 'Puedes elegir una de estas:', options: [ { label: 'Ver Cielorrasos', query: 'show-ceilings' }, { label: 'Cotizar un proyecto', query: 'quote' }, { label: 'Hablar con un asesor', query: 'contact' }, ]}};
  ...
  return (
    <div className="text-gray-700 font-sans">
      <header className="glassmorphic sticky top-2 sm:top-4 mx-auto max-w-7xl z-20 px-4">
        <div className="container mx-auto py-3 flex justify-between items-center gap-2 sm:gap-4">
          ...
          <div className="flex-1 flex items-center justify-end gap-2 flex-col sm:flex-row">
      ...
      <section id="icopor-gallery" className="py-12 sm:py-16">
          <div className="container mx-auto px-4 sm:px-6">
              <h2 className="text-2xl sm:text-3xl font-bold text-center text-gray-800 mb-4">Galería Interactiva de Diseños</h2>
              <p className="max-w-2xl mx-auto text-center text-gray-600 mb-8 sm:mb-12">Desliza para explorar nuestros diseños exclusivos. Pasa el cursor sobre una figura para verla en detalle.</p>
              <div className="flex items-center space-x-6 overflow-x-auto py-12 px-4 -mx-4" onMouseLeave={() => setHoveredIcoporIndex(null)}>
                  {icoporDesigns.map((src, index) => (
                      <div key={index} className={`flex-shrink-0 transition-all duration-300 ease-in-out cursor-pointer ${ hoveredIcoporIndex === index ? 'scale-150 z-10' : hoveredIcoporIndex !== null ? 'opacity-50 scale-90' : '' }`} onMouseEnter={() => setHoveredIcoporIndex(index)}>
                          <img src={src} alt={`Diseño de icopor ${index + 1}`} className="h-40 sm:h-48 w-auto object-contain" />
                      </div>
                  ))}
              </div>
          </div>
      ...
      <section id="tienda" className="py-12 sm:py-20">
        <div className="container mx-auto px-4 sm:px-6">
          <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-8 sm:mb-12 gap-4">
            ...
            <div className="neumorphic-card flex p-1 rounded-full self-center sm:self-auto"> <button onClick={() => setViewMode('grid')} className={`p-2 rounded-full ${viewMode === 'grid' ? 'neumorphic-button active' : ''}`}><GridViewIcon /></button> <button onClick={() => setViewMode('list')} className={`p-2 rounded-full ${viewMode === 'list' ? 'neumorphic-button active' : ''}`}><ListViewIcon /></button> </div>
          </div>
      ...
      <section id="about" className="py-10 bg-gray-100/50">
        <div className="container mx-auto px-4 sm:px-6 space-y-12 sm:space-y-16">
            ...
            <div className="grid md:grid-cols-2 gap-8 sm:gap-12 items-center">
             ...
             <div>
                <h3 className="text-xl sm:text-2xl font-bold text-center mb-8">Nuestros Valores Fundamentales</h3>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
                    {values.map(value => ( <div key={value.title} className="neumorphic-card p-4 sm:p-6 text-center"> <span className="text-4xl sm:text-5xl">{value.emoji}</span> <h4 className="mt-4 font-bold text-base sm:text-lg">{value.title}</h4> <p className="text-xs sm:text-sm text-gray-600 mt-1">{value.description}</p> </div> ))}
                </div>
            </div>
        </div>
      </section>

      <section id="testimonios" className="py-12 sm:py-20">
        <div className="container mx-auto px-4 sm:px-6">
          <h2 className="text-2xl sm:text-3xl font-bold text-center text-gray-800 mb-8 sm:mb-12">Lo que dicen nuestros clientes</h2>
      ...
      <section id="ubicacion" className="py-10">
        <div className="container mx-auto px-4 sm:px-6">
            <h2 className="text-2xl sm:text-3xl font-bold text-center text-gray-800 mb-8">Visítanos</h2>
            <div className="neumorphic-card overflow-hidden"> <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3975.69806981882!2d-75.6666986250284!3d4.821109940428612!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x8e388160e10d2937%3A0xb690a6e605d8f28c!2sMundo%20Icopor%20y%20Drywall!5e0!3m2!1ses!2sco!4v1721768817757!5m2!1ses!2sco" width="100%" height="450" style={{ border: 0 }} allowFullScreen loading="lazy" referrerPolicy="no-referrer-when-downgrade" title="Mapa de ubicación de Cielo rasos Mundo Icopor" ></iframe> </div>
        </div>
      </section>
      ...
      <footer className="glassmorphic mt-20 mb-4 mx-auto max-w-6xl text-gray-800">
       ...
       <div className={`cart-modal-overlay ${isCartOpen || isPolicyModalOpen || selectedProductForModal ? 'open' : ''}`} onClick={() => { setIsCartOpen(false); setIsPolicyModalOpen(false); setSelectedProductForModal(null); }}></div>
